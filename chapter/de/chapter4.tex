...
Gerät um einen (turing-)vollständigen Computer, auf dem das Betriebssystem 
...
wurde. Ein solcher Computer kann prinzipiell mit beliebiger zusätzlicher 
...
eines CD-Rippers für die Kommandozeile.
...
Kopien im eingeschränkten Rahmen einer sogenannten \textit{Privatkopie} erlaubt
sind.\\
...
einfache Installation möglich. Der Quellcode von \textit{abcde} befindet sich
...
unhörbaren Teile entfernt, um die resultierenden Dateien weiter zu verkleinern,
...
\verb|# Hier werden aus Platzgründen nur die aktiven Zeilen aufgelistet:|\\
...
\label{sect:rip}
Um eine CD zu rippen, legt man die gewünschte CD zunächst ins \CDROM-Laufwerk ein
...
Es sind keine Kommandozeilenparameter erforderlich, da ja alle gewünschten
...
ein Schreibfehler in den Datenbankeinträgen aus dem Internet (CDDB, musicbrainz)
entdeckt wurde. 
Neben den Zieldaten werden im Home-Verzeichnis des Anwenders in einem 
Unterverzeichnis temporäre Zwischendaten abgelegt:
...
Um dieses Verhalten zu umgehen, müssen vor dem Aufruf von \textit{abcde} alle
diese Hilfsverzeichnisse manuell gelöscht werden:
...
Coverbilder können in diesem Falle mit dem Browser über eine Suchmaschine Ihres
Vertrauens gesucht werden.\\
Am {\RPi} ist der installierte Chromium-Browser insbesondere bei der Suche nach
Bildern relativ langsam. Zudem ermöglicht das beim {\Bezeichnung} verwendete 
7-Zoll-Dis\-play mit seiner relativ geringen Auflösung von 800 x 480 Pixeln keine
besonders übersichtliche Ausgabe der Suchergebnisse. Dies kann nun entweder
...
Zur Ermittlung der Metadaten (Album, Künstler, Titel) greift \textit{abcde}
...
Meistens erhält man von \textit{MusicBrainz} gut gepflegte Datensätze zur
...
\textit{MusicBrainz} hinterlegt sind. Es spricht in solchen Fällen dafür,
sozusagen als Gegenleistung die Daten selbst auf \textit{MusicBrainz} zu
ergänzen.\\
Grundsätzlich muss für die Bearbeitung der \textit{MusicBrainz}-Datenbank
zunächst eine Benutzerkennung eingerichtet werden:\\
\url{https://musicbrainz.org/register?returnto=%2F}

Nun können die Daten auf \textit{MusicBrainz} bearbeitet werden. Die
Zuordnung einer physikalischen CD zu einem Musikalbum erfolgt über eine
...
Dies ist eine Software mit grafischer Oberfläche. Durch Anklicken der
Schaltfläche "`Lookup CD"' öffnet sich ein Fenster, in dem die alle Einträge
zur Disk-ID der eingelegten CD angezeigt werden. Bei fehlenden Daten auf
\textit{MusicBrainz} ist diese Liste leer. Ein Klick auf die Schaltfläche
"`Lookup in Browser"' öffnet auf dem {\RPi} den Chromium-Browser eine Seite
...
Dort kann z.B. im Textfeld "`Search by artist"' der Künstler eingegeben
werden. Anschließend werden  die Metadaten der CD hinterlegt. 
...
PC relativ langsam! Die Geschwindigkeit beim Rippen einer CD hängt jedoch in
...
schneller durchführt als ein herkömmlicher PC.
...
Dies zeigt, dass ein {\RPi} bei Verwendung vernünftiger Peripherie für 
bestimmte Anwendungen durchaus konkurrenzfähig ist.

Zum selbst zusammengestellten Gaming-PC ist kurz anzumerken, dass hier 
prinzipiell auf für's Gaming optimierte Komponenten Wert gelegt wurde. Da 
das \CDROM-Laufwerk jedoch keine kritische Rolle spielt, wurde das eher
günstige (aber offenbar bei Audio-CDs relativ langsame) LG-Modell GH24NSD1
eingebaut. Bei dieser spezifischen Anwendung (CDs rippen) bremst es den an
sich sehr flotten Gaming-PC stark aus.\\

Viel Spaß beim Rippen von CDs mit \textit{abcde} unter Linux am {\Bezeichnung}
